...
class BranchController extends Controller
{
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $query = Branch::query()->withCount('branchEmployee')->with('media', 'address', 'employee');
    ...
    public function branchData()
    {
        ...
        if (Auth::user()->hasRole('manager')) {
            $branch = Branch::where('id', auth()->user()->branch->branch_id)->with('address')->first();

            return response()->json(['data' => $branch, 'status' => true]);
        } else {
