<template>
  <div class="d-flex justify-content-between">
    ...
    <a href="javascript:void(0)" data-reset="body-heading-font" @click="resetFont">
  ...
  <div class="d-grid gap-3 grid-cols-2 mb-4">
    <div>
      ...
      <div class="form-group mb-0">
        <select className="form-control" @change="(e) => bodyFontFamily(e.target.value)" :value="`${bodyFontFamily !== null ? bodyFontFamily : 'Inter'}`">
    ...
    <div>
      ...
      <div class="form-group mb-0">
        <select className="form-control" @change="(e) => headingFontFamily(e.target.value)">
          <option v-for="(item, index) in options" :key="index" :value="item.value">{{ item.label }}</option>
        </select>
      </div>
    </div>
...
import fontList from '@/helpers/font.json'
export default {
  data() {
    return {
      options: fontList.items.map((font) => {
        return {
          label: font.family,
          value: font.family
        }
  ...
  setup() {
    ...
    const bodyFontFamily = (value) => {
      store.body_font_family(value)
    }
    const headingFontFamily = (value) => {
      store.heading_font_family(value)
    }
    const resetFont = (e) => {
      e.stopPropagation()
