...
<body class="{{ !empty(getCustomizationSetting('card_style')) ? getCustomizationSetting('card_style') : '' }} {{ auth()->user()->user_setting['theme_scheme'] ?? '' }}">
    ...
    <div class="main-content wrapper">
        <div class="position-relative @if(auth()->user()->hasRole('admin')) {{ !isset($isBanner) ? 'iq-banner' : '' }} default @endif">
            <!-- Header -->
        ...
        <div class="conatiner-fluid content-inner pb-0" id="page_layout">
            ...
            @if(isset($export_import) && $export_import)
              <div data-render="import-export">
                <export-modal export-url="{{ $export_url }}" :module-column-prop="{{ json_encode($export_columns) }}" module-name="{{ $module_name  }}"></export-modal>
